...
1. Design Files: These are the RTL files that an RTL design engineer writes.
2. PDKs: A Process Design Kit (PDK) is a set of files used to model a fabrication process. Generally, the PDK files are designed by a foundry with a certain technology.
...
- Library name: is a representation of the type of content found in the library. 
...
Picorv32a is a simple risc-v processor core present in the "design" directory. Before synthesizing the design prep the design so that it links to all the tech files and lib files for the design to be synthesized.
...
Ground bounce and voltage droop occur when a single voltage source is used for multiple logic cells. To avoid this multiple rails of VDD and VSS is used as shown in the figure below.
...
2. Go to "~/".
4. Run floorplan with the command "run_floorplan".You can run floorplan using the command "~/Desktop/work/t".
4. Run floorplan with the command "run_floorplan" You can run floorplan using the commnad "~/Desktop/work/tools/openlane_working_dir/openlane/configuration" to open the README.md file. This file consists of floorplan switches, below tables describe some of the switches for the user.
...
The aspect ratio is the ratio of height to width. Looking at "picorv32.floorplan.def" shown below shows the die area. The die is represented in a cartesian coordinate system, therefore the length is 389.440-micron units and the width is 746.405-micron units. Hence, the aspect ratio is 389.440/746.405 = 0.5215 ~ 0.5 as we set in the TCL file.
...
Each component in the netlist is represented as a box with a width and height. These blocks are present in the library with all the information regarding timing and area, and many more. Let's take an example of a netlist given in the figure below.
...
Using the cells in the library the given netlist will be placed on the floorplan as shown in the figure below.
...
7. Congestion related placement. Execute "run_placement" to run placement, global placement occurs with this command.
...
1. Technology library and setup environment: Technology libraries such as 5nm, 7nm, 14nm, etc are chosen for the fabrication process. Tools such as cadence, Synopsys, or mentor graphics are used to design the environment. 
...
Reset the variable and run the design again. Use the command "$set {varible}". For example to change the pin alignment from equidistant to stacked use => "set ::env(FP_IO_MODE) 2" and run floorplan again or any runs that are desired.
...
- Component Values: define the width/length of mos components, input load capacitance, and output load capacitance.
- Identify node: a node is the point where a component is connected to both ends. One component will be presented between two nodes.
...
From the tkcon window, ensure you are in the design location i.e., vsdcelldesign, perfom the following steps:
...
1. Change synthesis strategy more favorable to timing that area using  "set ::env(SYNTH_STRATERGY) "DELAY 3" "
...
1. Analyze the setup time, look for the maximum cell delay, and try to optimize the design.

Note: The delay of a cell is a function of the input slew and the output load. The delay of the cell increases with the input slew and the delay of the cell increases with the output load ie., the more the input slew, the more the delay, and the more the output capacitance is the delay.
...
- From the report, the first parameter to optimize is for fanout. Choose the fan to be no more than 4. To set the fanout during use "set ::env(SYNTH_MAX_FANOUT) 4" in the config file for convenience and synthesis. You can see that the negative slack is reduced.
...
- This is an iterative process where you find and optimize the path. One way is to increase the clock frequency to resolve timing violations, usually it is not preferred because clock frequency will be on of the constraints. For now, we can just increase the clock to resolve the issue. Changing the clock from 12 to 24.73 results in a positive slack.
...
- Run the following commands after running cts from the terminal:
...
- Follow the commands to execute the design:
...
1. Use the command "run_routing" to perform detailed routing. Used routing strategy 0 so that it completes soon. The images shown below are the result of the routing.
